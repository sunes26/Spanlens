...
8. **🔥 Vercel Edge fire-and-forget 금지 — 반드시 `fireAndForget()` 사용**: `logRequestAsync(...).catch(console.error)` 패턴은 **Vercel Edge runtime에서 pending promise를 통째로 drop**함 → 프록시 200 응답은 내려가는데 DB `requests` INSERT 조용히 사라짐. 로컬 Node dev / 직접 curl은 우연히 성공해서 테스트에 안 잡히고 production에서만 데이터 유실 — 가장 위험한 종류의 버그. 해결: `apps/server/src/lib/wait-until.ts`의 `fireAndForget(c, promise)` 사용 (`@vercel/functions` `waitUntil` 래퍼, Edge+Node 모두 drain 보장). `c.executionCtx`는 Hono getter가 없는 환경에서 **접근만 해도 throw**하므로 직접 쓰지 말 것. proxy/openai.ts, anthropic.ts, gemini.ts 참고.
   - **`apps/server/api/index.ts`는 Edge runtime (`runtime = 'edge'`)**: 한때 Node 전환 시도했으나(commits efc3fde→2b57b01) `app.fetch` 직접 export가 Vercel Node에서 안 동작해서 handler 깨져 reverted. Edge로 안정 운영 중이지만 first-byte 25s 한계 있음 → 긴 요청은 **gotcha #11의 internal streaming** 으로 우회. Node 재시도는 ROADMAP 3F의 트리거(고객 504율 1%+ 또는 timeout 불만 5건+) 발생 시 `hono/vercel`의 `handle()` 어댑터로만 진행.
...
11. **Spanlens 프록시 25초 first-byte timeout — 긴 요청은 internal streaming 패턴 권장**: `apps/server/api/index.ts`는 현재 Edge runtime이라 25초 안에 첫 byte 안 내려가면 504. JSON mode + 큰 `max_tokens`는 단번에 25초 초과 가능. 해결책: **internal streaming** — 서버는 `stream:true`로 OpenAI 호출(첫 byte ~200ms), chunk 누적, 클라이언트엔 단일 JSON 반환. 클라이언트 contract 안 변하면서 timeout 우회. 참고 구현: mind-scanner `app/api/analyze/route.ts`. `/docs/proxy` + `/docs/sdk` 상단 amber 배너로 안내 중. (장기적 Node runtime 전환은 ROADMAP 3F — 트리거 발생 시 `hono/vercel`의 `handle()` 어댑터 사용, 직접 `app.fetch` export 금지 — 2026-04-22 시도했다 깨진 패턴.)
...
13. **🔥 Edge runtime은 `node:crypto`/`node:buffer` import 불가 — 반드시 `lib/crypto.ts`의 헬퍼 사용**: `apps/server/api/index.ts`가 Edge runtime이라 Vercel 빌드 시 *"The Edge Function "api/index" is referencing unsupported modules"* 로 reject. invitations.ts에서 `import { randomBytes, createHash } from 'node:crypto'` 썼다가 production 배포 깨짐 (commit 0b5470b). 대안: `randomHex(32)` (sync) / `sha256Hex(text)` (async) — 둘 다 Web Crypto 기반이라 Edge·Node 모두 호환. 신규 보안/암호화 코드는 `node:`로 시작하는 import가 있는지 검토 필수.
